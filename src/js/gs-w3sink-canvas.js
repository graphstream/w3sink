(function(exports) {
	...
	function CanvasNode(graph, nodeId) {
		GS.Node.call(this, graph, nodeId);
	...
	GS.extend(GS.Node.prototype, CanvasNode.prototype);

	function CanvasEdge(graph, edgeId, source, target, directed) {
		GS.Edge.call(this, graph, edgeId, source, target, directed);
	...
	GS.extend(GS.Edge.prototype, CanvasEdge.prototype);	
	...
	function Layers(stage, nodeLayers, edgeLayers) {
		...
		for (var i = 0; i < edgeLayers; i++) {
			var l = new Kinetic.Layer("edges#"+i);
			l.changed = true;
			this.edges.push(l);
			this.stage.add(l);
		}
		...
		for (var i = 0; i < nodeLayers; i++) {
			var l = new Kinetic.Layer("nodes#"+i);
			l.changed = true;
			this.nodes.push(l);
			this.stage.add(l);
		}
	...
	Layers.prototype.dragStart = function(node) {
		...
		for (var eid in node.edges) {
			var l = node.edges[eid].shape.getLayer();
	...
	exports.GS.extend(GS.Context.prototype, CanvasContext.prototype);	
