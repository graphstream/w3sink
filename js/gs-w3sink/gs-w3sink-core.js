(function(exports) {
	...
	Element.prototype = {
		...
		setStyle: function(style) {
			...
			for (var i = 0; i < styles.length; i++) {
				...
				case 'arrow-size':
				case 'shape-points':
					console.log("[GraphStream CSS] unsupported property '"+t[0]+"' for " + this.type);
					break;
				default:
					console.log("[GraphStream CSS] unknown property '"+t[0]+"' for " + this.type);
					break;
				}
			}
		},
	...
	function Edge(graph, id, source, target, directed) {
		...
		this.points = [{x:source.pixelX,y:source.pixelY}, {x:target.pixelX,y:target.pixelY}];
	...
	Graph.prototype = {
		...
		an: function(id) {
			if (this.nodes.hasOwnProperty(id)) {
				console.log("[warning] node exists '" + id + "'");
				return;
			}
	...
	Context.prototype = {
		...
		createNode: function(graph, nodeId) {return new Node(graph, nodeId);},
		removeNode: function(graph, node) {},
		createEdge: function(graph, edgeId, source, target, directed) {return new Edge(graph, edgeId, source, target, directed);},
