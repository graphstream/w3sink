...
public class W3SinkDemo {
	...
	public W3SinkDemo() throws IOException {
		...
		this.layout = new SpringBox();
		this.heartbeat = new HeartBeat(60000);
		
		this.g.setStrict(false);
		this.g.addSink(this.linkManager);
		this.g.addSink(this.layout);
		this.g.addSink(this.heartbeat);
		this.layout.addAttributeSink(this.g);
		
		PrintStream out = new PrintStream("verbose.sink");
		this.g.addSink(new VerboseSink(out));
	...
	private void handleGraph() throws UnknownHostException, IOException {
		...
		while (alive) {
			...
			if (layout.getStabilization() < 0.95)
				layout.compute();
	...
	public static final String SAVE_PATH = W3SinkDemo.class.getName() + "-save.dgs";
